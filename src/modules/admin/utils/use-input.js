...
useEffect(() => {
  ...
  if (defaultValue?.type === "number") {
    dispatch({
      ...
      value: defaultValue.value,
      data: {
        dialCode: defaultValue.dialCode || "",
        phoneNumber: defaultValue.rawPhone || ""
      }
    });
  }
}, [defaultValue?.type,
  ...
  defaultValue?.rawPhone]); // Watch entire defaultValue object
  ...
  const reactPhoneChangeHandler = (value, data, e, formattedValue) => {
    ...
    // const cleanedValue = value.replace(/[^0-9+]/g, ""); // Remove non-numeric chars
    // dispatch({
    //   type: "PHONE",
    //   value: cleanedValue,
    //   data: data,
