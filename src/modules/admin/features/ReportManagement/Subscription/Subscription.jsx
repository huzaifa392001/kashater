...
const Subscription = () => {
  ...
  const onDownloadItemClick = fileType => {
    ...
    axios({
      method: "post",
      url: `${baseUrl}/admin/report-management/subscription-report/report-subscription-plan-list-download`,
      data: {
        type: fileType,
        search: searchValue,
        // filter: {
        plan: category, // active,inactive
      ...
      .then(response => {
        ...
        let fileName = `${"Subscription_reports"}.${fileType}` // default file name with correct extension
        ...
        if (contentDisposition) {
          ...
          fileName = `${"Subscription_reports"}.${fileType}`
        }
  ...
  const viewList = () => {
    sendRequest(
      {
        url: `admin/report-management/subscription-report/report-subscription-plan-list`,
        method: "POST",
        body: {
          page: page,
          per_page: perPage,
  ...
  const apiBoxesData = () => {
    sendRequest(
      {
        url: `admin/report-management/subscription-report/report-subscriptions`,
      },
      data => {
    ...
    <>
      <div className={classes.scanOut} style={{ marginBottom: "3rem" }}>
        ...
        <div style={{ marginBottom: " 2rem" }}>
          <CustomTable
            data={data}
            columns={subscriptionReportData}
            loader={false}
            onSort={handleSort}
