...
const FeaturedBooks = () => {
  ...
  const handleDragEnd = (sourceData, destinationData, event) => {
    ...
    updateBookOrder(
      {
        method: "POST",
        url: `admin/story-management/library-config/update-order/featured_month_books/${sourceData.id}`,
        body: {
          ...
          children={
            <>
              <div className="dialog_loyout">
                ...
                <div>
                  {moduleList.length > 0 && (<p className={classes.module_text}>Choose Books</p>)}
                  {/* {!selectedItemsError ? "" : ""} */}
